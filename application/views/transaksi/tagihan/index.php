                ...
                <div class="card bg-light">
                    ...
                    <?php if ($t->status == "BELUM_LUNAS") { ?>
                        <div class="card-footer">
                            <button type="button" title="Batal Pembayaran" onclick="batal_pemesanan('<?= $t->id ?>', '<?= $t->id_user ?>')" class="btn btn-sm btn-danger"><i class="fas fa-times"></i> BATALKAN PEMESANAN</button>
                            <button type="button" title="Verifikasi Pembayaran" onclick="verifikasi_pemesanan('<?= $t->id ?>', '<?= $t->id_user ?>')" class="btn btn-sm btn-success"><i class="fas fa-check"></i> VERIFIKASI PEMESANAN</button>
                        </div>
                    <?php } ?>
                </div>
            <?php } ?>
...
<script>
    function batal_pemesanan(id, id_user) {
        swal.fire({
    ...
    function verifikasi_pemesanan(id, id_user) {
        swal.fire({
            title: 'Verifikasi pembayaran #' + id + '?',
            text: "Pesanan yang sudah diverifikasi akan masuk ke list barang dikemas",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonText: 'Tutup',
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('transaksi/tagihan/verifikasi_pemesanan') ?>",
                    data: {
                        "id": id,
                        "id_user": id_user,
                    },
